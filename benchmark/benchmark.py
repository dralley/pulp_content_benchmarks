...
import time
import django
import models_benchmark
from django.conf import settings
settings.configure(default_settings=models_benchmark.settings, DEBUG=True)
django.setup()
...
def main():
    ...
    flat_content = make_content(FlatFileContent)
    inherited_content = make_content(InheritedFileContent)
    ...
    start = time.time()
    InheritedFileContent.objects.bulk_create(flat_content)
    end = time.time()
